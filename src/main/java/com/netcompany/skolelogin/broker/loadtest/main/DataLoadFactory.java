...
public class DataLoadFactory {
  ...
  public void produceLoadTestFiles() throws IOException {
    initialize();
  ...
  private void writePersonData() throws IOException {
    List<Aktoer> personer = new ArrayList<>();

    int elevStartår = 2000;
    int elevSlutår = 2010;
    int antalEleverPrDag = 1;
      ...
      for (int måned = 1; måned <= 12; måned++) {
        ...
        for (int dag = 1; dag <= månedslængde; dag++) {
          for (int i = 1; i <= antalMedarbejderePrDag; i++) {
            ...
            medarbejderTæller++;
            personer.add(createMedarbejder(år, måned, dag, i, uniId));
          }
        }
      }
    }
    ...
    int kontaktpersonEtBarnSlutår = 1960;
      ...
      for (int måned = 1; måned <= 12; måned++) {
        ...
        for (int dag = 1; dag <= månedslængde; dag++) {
          for (int i = 1; i <= antalKontaktpersonerMedEtBarnPrDag; i++) {
            ...
            personer.addAll(createKontaktperson(år, måned, dag, i, elev));
          }
        }
      }
    }
    ...
    int kontaktpersonToBørnSlutår = 1965;
      ...
      for (int måned = 1; måned <= 12; måned++) {
        ...
        for (int dag = 1; dag <= månedslængde; dag++) {
          for (int i = 1; i <= antalKontaktpersonerMedToBørnPrDag; i++) {
            ...
            personer.addAll(createKontaktperson(år, måned, dag, i, barn1, barn2));
          }
        }
      }
    }
    ...
    for (int måned = 1; måned <= 12; måned++) {
      ...
      for (int dag = 1; dag <= månedslængde; dag++) {
        Collections.shuffle(elever);
        Elev elev = elever.get(0);
        personer.addAll(createMedarbejderOgKontaktperson(medarbejderOgKontaktpersonÅrgang, måned, dag, 1, elev));
      }
    }
    ...
    for (int måned = 1; måned <= 12; måned++) {
      int månedslængde = YearMonth.of(medarbejderOgKontaktpersonÅrgang, måned).lengthOfMonth();
      for (int dag = 1; dag <= månedslængde; dag++) {
        personer.addAll(createElevOgMedarbejder(elevOgMedarbejderÅrgang, måned, dag, 1));
      }
    }

    System.out.println("Antal aktører: " + personer.size());
    ObjectMapper objectMapper = new ObjectMapper();

    int chunkSize = 200;
    int chunkNumber = 1;

    while (chunkNumber * chunkSize < personer.size()) {
      String fileNumber = "" + chunkNumber;
      while (fileNumber.length() < 4) {
        fileNumber = "0" + fileNumber;
      }
      File file = new File(".\\persondata\\persondata_" + fileNumber + ".json");
      FileOutputStream os = new FileOutputStream(file);

      List<Aktoer> chunk = personer.subList((chunkNumber - 1) * chunkSize, chunkNumber * chunkSize);

      Writer w = new BufferedWriter(new OutputStreamWriter(os, StandardCharsets.UTF_8));
      w.write(objectMapper.writerWithDefaultPrettyPrinter().writeValueAsString(chunk));
    ...
    List<Aktoer> chunk = personer.subList((chunkNumber - 1) * chunkSize, personer.size() - 1);
  ...
  private void writeGroupData() throws IOException {
    ...
    while (chunkNumber * chunkSize < grupper.size()) {
      String fileNumber = "" + chunkNumber;
      while (fileNumber.length() < 4) {
        fileNumber = "0" + fileNumber;
      }

      File file = new File(".\\groupdata\\groupdata" + fileNumber + ".json");
      FileOutputStream os = new FileOutputStream(file);

      List<Gruppe> chunk = grupper.subList((chunkNumber - 1) * chunkSize, chunkNumber * chunkSize);
    ...
    List<Gruppe> chunk = grupper.subList((chunkNumber - 1) * chunkSize, grupper.size() - 1);
  ...
  public void initialize() throws IOException {
    ...
    elevTæller = 1;
    kontaktpersonTæller = 1;
    medarbejderTæller = 1;
    medarbejderOgKontaktpersonTæller = 1;
    elevOgMedarbejderTæller = 1;
  }
